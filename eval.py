...
def load_ground_truth( gt_file: str,
    ...
    if verify: verify_ground_truth_format(gt_raw)
    ...
    if is_linking: add_group_info(gt, is_e2e=is_e2e, is_gt=True)
...
def load_predictions( preds_file: str,
    ...
    if is_linking: add_group_info(preds, is_e2e=is_e2e, is_gt=False)
...
def flatten_zip_dict( gt: dict[str,ImageData],
    ...
    if not 'link' in task: # flatten entries to lists of words for evaluation
